...
def test_update_charity_project(
    superuser_client, charity_project, json, expected_data
):
...
def test_update_charity_project_invalid(
    superuser_client, charity_project, json
):
    response = superuser_client.patch('/charity_project/1', json=json)
    assert response.status_code == 422, (
...
def test_get_all_charity_project_not_auth_user(
